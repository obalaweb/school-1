<section class="section-md pbmit-element-gallery-style-1">
    <div class="container">
        <div class="row g-0">
            <div class="col-md-9 col-xl-10">
                <div class="pbmit-heading-subheading">
                    <h4 class="pbmit-subtitle">Gallery</h4>
                    <h2 class="pbmit-title">Kids school you’ll love</h2>
                </div>
            </div>
            <div class="col-md-3 col-xl-2">
                <div class="pbmit-head-btn">
                    <a href="services.html" class="pbmit-btn">
                        <span>Full Gallary</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-9">
                <div class="row">
                    <div class="col-md-8">
                        <div class="pbmit-gallery-thumb">
                            <a href="{{ asset('assets/images/homepage-1/gallry-01b.jpg') }}" class="pbmit-lightbox">
                                <img src="{{ asset('assets/images/homepage-1/gallry-01.jpg') }}" class="img-fluid"
                                    alt="">
                            </a>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="pbmit-gallery-thumb">
                            <a href="{{ asset('assets/images/homepage-1/gallry-02b.jpg') }}" class="pbmit-lightbox">
                                <img src="{{ asset('assets/images/homepage-1/gallry-02.jpg') }}" class="img-fluid"
                                    alt="">
                            </a>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        <div class="pbmit-gallery-thumb">
                            <a href="{{ asset('assets/images/homepage-1/gallry-04b.jpg') }}" class="pbmit-lightbox">
                                <img src="{{ asset('assets/images/homepage-1/gallry-04.jpg') }}" class="img-fluid"
                                    alt="">
                            </a>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="pbmit-gallery-thumb">
                            <a href="{{ asset('assets/images/homepage-1/gallry-05b.jpg') }}" class="pbmit-lightbox">
                                <img src="{{ asset('assets/images/homepage-1/gallry-05.jpg') }}" class="img-fluid"
                                    alt="">
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="pbmit-gallery-thumb">
                    <a href="{{ asset('assets/images/homepage-1/gallry-03b.jpg') }}" class="pbmit-lightbox">
                        <img src="{{ asset('assets/images/homepage-1/gallry-03.jpg') }}" class="img-fluid"
                            alt="">
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>
